...
def load_ventes(df: pd.DataFrame, engine) -> int:
    ...
    df.to_sql("ventes_propres", engine,
...
def load_mart(df: pd.DataFrame, engine) -> int:
    ...
    df.to_sql("ca_par_mois", engine,
              if_exists="replace", index=False, method="multi")
    return len(df)


def load_mart_category(df: pd.DataFrame, engine) -> int:
    """Charge le DataFrame agrege par categorie dans ca_par_categorie. (Bonus)"""
    df.to_sql("ca_par_categorie", engine,
              if_exists="replace", index=False, method="multi")
    return len(df)
